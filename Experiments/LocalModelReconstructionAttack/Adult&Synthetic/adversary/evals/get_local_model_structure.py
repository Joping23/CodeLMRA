...
from federated_learning.model.neural_network_regression import NeuralNetworkReg
...
def get_local_model_structure(model, local_data_dir):
    if model == "linear" or model == "neural":
        ...
        if model == "neural":
            return NeuralLayer(num_dimension, 32, num_classes), num_classes, num_dimension
    elif model == "mlp":
        return NeuralLayer(32*32*3, 256, 10), 10, 32*32*3
    elif model == "conv":
...
def map_vector_to_net(adversary_local_model, net, num_classes, num_dimension, model):
    if model == "linear":
        ...
        for param in net.parameters():
            ...
            iter += 1
    elif model == "neural" or model == "mlp" or model == "conv":
        start = 0
        for param in net.parameters():

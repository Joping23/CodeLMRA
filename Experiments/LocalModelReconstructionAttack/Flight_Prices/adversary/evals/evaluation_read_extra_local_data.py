...
def load_extra_local_data(worker_id, data_directory, dim=10, num_samples=10000, sigma=20, num_classes=2):
    np.random.seed(0)
    ...
    # Generate y
    prob = softmax(np.matmul(samples, w) + np.random.normal(loc=0., scale=sigma, size=(num_samples, num_classes)), axis=1)
